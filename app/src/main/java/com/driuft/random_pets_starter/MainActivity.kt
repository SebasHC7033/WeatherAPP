...
class MainActivity : AppCompatActivity() {
    ...
    private fun getWeatherData(cityLocation: TextView, temperature: TextView, type: TextView, icon: ImageView, mmtemp: TextView, userInput: String?) {
            ...
            fusedLocationClient.lastLocation
                .addOnSuccessListener { location: Location? ->
                    location?.let {
                        ...
                        if (userInput != null){
                            currentWeatherUrl = "https://api.openweathermap.org/data/2.5/weather?id=$userInput&appid=$apiKey&units=metric"
                        }
                        client.get(
                            currentWeatherUrl,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult?) {
                locationResult?.lastLocation?.let { location ->
                    ...
                    getWeatherData(cityLocation, currentTemp, currentWeather, weatherIcon, minmaxTemp, null)
                }
            }
        }
        ...
        findViewById<Button>(R.id.button)
            .setOnClickListener {
                ...
                val input: String = cityInput.text.toString().trim().lowercase()
                if (input == "newyork"){
                    getWeatherData(cityLocation, currentTemp, currentWeather, weatherIcon, minmaxTemp, "5128581")
                }
            }
